...
from db import db, User, Sessions, RecoverAccountDB, Logins
...
def whoami():
    ...
    return jsonify({
        ...
        'data':  {
            ...
            'name': user.username,
            'email': user.email,
            'role': [i.role.name for i in user.userrole_ref.all()],
            'phone': user.phone,
...
def login():
    ...
    lookupinfo = json.loads(requests.get(
        f"http://ipwho.is/{request.remote_addr}").text)
...
def get_login():
    ...
    ret = db.session.query(Logins).filter(Logins.userid == request.session.user.id).order_by(
        Logins.created_on.desc()).offset(p*10).limit(10).all()
...
def reset_password():
    ...
    user = db.session.query(User).filter(
        User.email == data['email'].strip()).first()
...
def resend_code():
    ...
    rec = db.session.query(RecoverAccountDB).filter(
        RecoverAccountDB.unique_identifier == data['uuid'].strip()).first()
...
def confirm_reset_passwd():
    ...
    if is_urltoken_present:
        rec = query.filter(RecoverAccountDB.url_token ==
                           data['url_token'].strip()).first()
    elif is_code_present:
        rec = query.filter(RecoverAccountDB.recover_code == data['code']).filter(
            RecoverAccountDB.unique_identifier == data['uuid'].strip()).first()
...
def reset_new_password():
    ...
    rec = db.session.query(RecoverAccountDB).filter(
        RecoverAccountDB.url_token == data['token'].strip()).first()
