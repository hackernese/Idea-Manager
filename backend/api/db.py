...
class User(AbstractBase):
    ...
    email = db.Column(db.String(345), unique=True, nullable=False) # Maximum length of email is 345 chars
    ...
    __table_args__ = (
        CheckConstraint(gender.in_(['male', 'female', 'others', 'unknown']), name='gender_con'),
    ...
    idea_ref = db.relationship('Idea', backref='user', lazy="dynamic", cascade='all, delete')
    userrole_ref = db.relationship('UserRoles', backref='user', lazy="dynamic", cascade='all, delete')
...
class Sessions(AbstractBase):
    ...
    expiry_time = db.Column(db.DateTime, default=datetime.now() + timedelta(days=TOKEN_EXPIRY_DAYS))
...
class Category(AbstractBase):
    ...
    idea_ref = db.relationship("Idea", backref='category', lazy=True, cascade='all, delete')
...
class Submission(AbstractBase):
    ...
    reference = db.relationship('Idea', backref='submission', lazy=True, cascade='all, delete')
...
class Department(AbstractBase):
    ...
    reference = db.relationship('User', backref='department', lazy=True, cascade='all, delete')
...
class Role(AbstractBase):
    ...
    userrole_ref = db.relationship("UserRoles", backref='role', lazy=True, cascade='all, delete')
...
def insert_default_record(*args, **kwargs):
    db.session.add_all([Role(name=i['r'], mandatory=i['m']) for i in DEFAULT_ROLE_LIST])
    db.session.commit()
...
def insert_default_record(*args, **kwargs):
    ...
    for record in DEFAULT_USER_LIST:
        ...
        department = db.session.query(Department).filter(Department.name == record['department'] ).first()
        role = db.session.query(Role).filter(Role.name==record['role']).first()
